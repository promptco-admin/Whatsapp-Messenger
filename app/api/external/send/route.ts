...
import { logActivity, logError } from "@/lib/audit";

export const dynamic = "force-dynamic";

/**
 * Server-to-server equivalent of /api/messages/send, for Prompt-Solar CRM's
 * edge functions (call-logged notifications, the finalize/approval PDF).
 * Same request shape and same messages-table logging as the authenticated
 * composer send, so these show up in the normal chat history — just
 * authenticated with CRM_SERVICE_KEY instead of a browser session, and
 * sent_by_user_id is left null (no human agent sent it).
 */
  ...
  try {
    if (kind === "text") {
      ...
      logActivity({
        ...
        contactId,
        summary: `[CRM] Sent text to +${normalized}: "${text.slice(0, 80)}${text.length > 80 ? "…" : ""}"`,
        metadata: { kind: "text", length: text.length, source: "prompt-solar-crm" },
      });
    ...
    if (kind === "template") {
      ...
      const res = db()
        .prepare(
          `INSERT INTO messages (wa_message_id, contact_id, direction, type, body, template_name, template_variables, status, media_id, media_url, media_mime, media_filename)
      ...
      logActivity({
        ...
        entityId: Number(res.lastInsertRowid),
        contactId,
        summary: `[CRM] Sent template "${name}" to +${normalized}`,
        metadata: { kind: "template", template: name, language, variables, source: "prompt-solar-crm" },
      });
      return NextResponse.json({ id: res.lastInsertRowid, messageId });
    }
    ...
    db()
      .prepare(
        `INSERT INTO messages (contact_id, direction, type, body, template_name, status, error)
